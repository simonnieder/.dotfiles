...
Existing cloned repos from the old librarian skill live under:

`~/.cache/checkouts`

Use that as the primary place to search/reuse repositories when available.
...
<workflow>
    <step name="work dir setup">
        use ~/.cache/checkouts as the primary place where existing repos may already be cloned from the old librarian skill
    </step>
    <step name="load">
        first check ~/.cache/checkouts for an existing clone and use that if present. if the repo is not available there, use ~/.btca/agent/sandbox as the place where you clone/search repos. clone the main branch by default, unless the user asks for something else
    </step>
    <step name="search">
...
Your job is to search ~/.cache/checkouts first, and then ~/.btca/agent/sandbox if needed, just to get a list of all the repos that have been previously cloned
