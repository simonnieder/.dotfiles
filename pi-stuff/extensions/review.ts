...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	function parseStandaloneQuotedInstruction(args: string): string | undefined {
	...
	function parseArgs(args: string | undefined): ParsedReviewArgs {
		...
		const rawParts = tokenizeArgs(trimmedArgs);
		const parts: string[] = [];
		...
		switch (subcommand) {
			...
			default: {
				const standaloneInstruction = !extraInstruction ? parseStandaloneQuotedInstruction(trimmedArgs) : undefined;
				return { target: null, extraInstruction: standaloneInstruction ?? extraInstruction };
			}
		}
	}
